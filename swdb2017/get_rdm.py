...
def get_rsms(boc, targeted_structure,cre_line,imaging_depth,stimuli):
    ''' Gives back rsms
    ...
    '''
    rsms = {}
    experiment_ids=get_experiment_ids(boc,targeted_structure,cre_line,imaging_depth) #self.experiment_ids
    for experiment_id in experiment_ids:
        ...
        if stimuli[0] == 'natural_scenes':
            ...
            rsms=ns.representational_similarity

        elif stimuli[0] == 'drifting_gratings':
            dg=DriftingGratings(data_set)
            rsms=dg.representational_similarity
...
rsms1=pd.DataFrame(get_rsms(boc, targeted_structure,cre_line,imaging_depth,stimuli))
rsms2=pd.DataFrame(get_rsms(boc, targeted_structure,cre_line,[275],stimuli))



rsms1.to_hdf(rsms_RL.h5,'rsms_RL')
rsms2.to_hdf(rsms_RL.h5,'rsms_RL')
